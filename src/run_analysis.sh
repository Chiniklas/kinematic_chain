...
PYTHONPATH="${project_dir}/src" MPLCONFIGDIR="${TMPDIR:-/tmp}/kinematic-chain-matplotlib" \
  python3 "${project_dir}/src/workspace_sweep.py" \
  "${abstraction}" \
  --output "${output_dir}/workspace_report.png" \
  --csv "${output_dir}/workspace_samples.csv"

PYTHONPATH="${project_dir}/src" MPLCONFIGDIR="${TMPDIR:-/tmp}/kinematic-chain-matplotlib" \
  python3 "${project_dir}/src/torque_analysis.py" \
  "${abstraction}" \
  --output "${output_dir}/torque_report.png" \
  --csv "${output_dir}/torque_samples.csv"
