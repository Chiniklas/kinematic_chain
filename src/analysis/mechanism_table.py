...
def render_markdown(data: dict[str, Any], source_path: Path) -> str:
    ...
    lines = [
        ...
        *([
            ...
            "",
            f"Reference: `{data['human_hand_model'].get('reference_finger', 'unspecified')}`; "
            f"pose: `{data['human_hand_model'].get('pose', 'unspecified')}`.",
            "",
            *_markdown_table(
            ...
            *_markdown_table(
                ...
                [[row.get("id"), row.get("joints"), row.get("length_mm"),
                  row.get("width_mm"), row.get("shape")]
                 for row in [data["human_hand_model"]["palm"],
                             *data["human_hand_model"].get("phalanges", [])]],
            ),
            "",
            "## Exoskeleton attachments",
            "",
            *_markdown_table(
                ["Attachment", "Mechanism", "Hand reference", "Hand interface",
                 "Connector", "Dorsal clearance [mm]"],
                [[row.get("id"), row.get("mechanism_node"), row.get("hand_reference"),
                  row.get("hand_interface"), row.get("connector", row.get("location")),
                  row.get("dorsal_clearance_mm", "—")]
                 for row in data.get("exoskeleton_attachments", [])],
            ),
            "",
        ] if isinstance(data.get("human_hand_model"), dict) else []),
        "## Model readiness",
        "",
        *_markdown_table(
            ["Analysis", "Status"],
